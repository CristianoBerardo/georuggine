...
Come prima cosa vengono aggiornati i dati prima della visualizzazione a schermo:
...
3. Lettura dei nomi degli utenti connessi.
...
└───────────────────────────┘           │
┌───────────────────────────┐           │                           ┌───────────────────────────┐
│ handle_connection (B)     │ ──────────├────────chat_tx/rx────────▶│ main_ui (task principale) │
│  invia ChatMessage        │           │                           │  aggiorna la chat log     │
└───────────────────────────┘           │                           └───────────────────────────┘
┌───────────────────────────┐           │
│ handle_connection (…)     │ ──────────┘
│  invia ChatMessage        │
...
Entrambi i canali hanno **più produttori** (uno per ogni client connesso, per la chat; i vari handler e `network.rs` per gli errori) e un solo consumatore, la TUI, esattamente come il canale `client_error_tx` lato client (3.5) — con la differenza che qui i produttori sono task diversi per ogni connessione attiva, anziché due sole task fisse.
...
| [`main.rs`](server/src/main.rs)                                                       | Entry point del server: si occupa di creare stato e canali e di iniettarli nel resto dell'app. Nello specifico, inizializza il logger CPU, apre il database SQLite, istanzia lo stato condiviso `AppState` con i relativi canali (broadcast, watch, mpsc), avvia il listener TCP in background e avvia l'interfaccia terminale TUI.                           |
| [`network.rs`](server/src/network.rs)                                                 | Gestisce l'ascolto di rete TCP: si lega all'indirizzo configurato, notifica l'avvio tramite canale oneshot, accetta le connessioni in ingresso dai client in un loop asincrono e genera per ciascuna un task Tokio dedicato a `handle_connection`. Viene generato un task per connessione, cosicché un client lento o in errore non possa bloccare gli altri. |
| [`state.rs`](server/src/state.rs)                                                     | Definisce lo stato globale condiviso `AppState` (pool SQLite che contiene di suo un `Arc` per essere condiviso in maniera thread safe, mappa delle connessioni attive protetta da `RwLock`, stato utenti in tempo reale e canali di notifica/shutdown) e le strutture dati correlate (`UserStatus`, `Info`, `IncomingChat`, `IncomingError`).                 |
...
| [`auth.rs`](server/src/auth.rs)                                                       | Modulo per la sicurezza e l'hashing delle credenziali: implementa `hash_password` e `verify_password` basate su algoritmo crittografico Argon2 e salt casuale (`OsRng`), garantendo che le password non siano mai memorizzate in chiaro nel database.                                                                                                         |
...
| [`handlers/handle_delete_account.rs`](server/src/handlers/handle_delete_account.rs)   | Gestisce la cancellazione dell'account (`DeleteAccount`) in tutte le sue parti: verifica la password tramite hash su DB, rimuove l'utente e i suoi punti GPS da SQLite, cancella la connessione attiva e lo stato in memoria e conferma l'eliminazione al client.                                                                                             |
...
| [`logging/cpu_logger.rs`](server/src/logging/cpu_logger.rs)                           | Monitora le prestazioni della CPU in un thread di background separato: campiona ogni 120s l'uso CPU del processo (normalizzato sui core) e globale tramite `sysinfo`, salvando i dati nel file `cpu_metrics.log`. Queste operazioni vengono svolte da un thread di sistema, non un task Tokio, così il campionamento sincrono non blocca l'event loop async.  |
| [`ui/mod.rs`](server/src/ui/mod.rs)                                                   | Dichiara i sottomoduli `main_ui`, `size_control`, `terminal_guard`.                                                                                                                                                                                                                                                                                           |
| [`ui/main_ui.rs`](server/src/ui/main_ui.rs)                                           | Coordina l'event loop della TUI con `tokio::select!`: sincronizza lo stato locale da `AppState`, ridisegna l'interfaccia e gestisce concorrentemente eventi tastiera, chat in arrivo dai client, errori di rete e variazioni delle connessioni. L'idea di base è implementare un'attesa passiva su più sorgenti al posto del polling attivo.                  |
...
| [`ui/main_ui/state.rs`](server/src/ui/main_ui/state.rs)                               | Definisce i modelli e lo stato interno della TUI (`App`): memorizza i log di chat e broadcast, la cronologia errori, i buffer di input testo, il pannello correntemente selezionato (`Panel`) e lo stato delle statistiche (`StatsStep`). È da `App` che `draw.rs` ricava le informazioni da stampare a schermo.                                              |
| [`ui/size_control.rs`](server/src/ui/size_control.rs)                                 | Definisce le dimensioni minime del terminale (`MIN_WIDTH`/`MIN_HEIGHT`) e la funzione che verifica se lo spazio disponibile è sufficiente, usata per mostrare l'avviso di terminale troppo piccolo. È una sorta di guardia controllata a ogni frame prima di disegnare i pannelli.                                                                            |
...
### 3.3 Inizializzazione delle schermate (auth_ui.rs e main_ui.rs)

Sia [`ui::auth_ui::run`](client/src/ui/auth_ui.rs) sia [`ui::main_ui::run`](client/src/ui/main_ui.rs) iniziano con la stessa sequenza di inizializzazione, prima di entrare ciascuna nel proprio loop:

1. Creazione dello stato della schermata (`App::new()`): il form di login/registrazione per `auth_ui`, i riquadri/log/stato movimento per `main_ui`.
2. Inizializzazione di `ratatui` (`ratatui::init()`).
3. Creazione della guardia RAII [`TerminalGuard`](client/src/ui/terminal_guard.rs): il suo `Drop` ripristina il terminale anche in caso di uscita anomala o panic.
4. Creazione di un `EventStream` di `crossterm` per ricevere gli eventi da tastiera in modo asincrono, senza bloccare il thread.

La sequenza è identica nei due file: lo stesso pattern si ripete in entrambi perché ogni schermata gestisce in autonomia il proprio terminale e il proprio ciclo di eventi.

### 3.4 Ciclo principale (ui/main_ui.rs)

Una volta autenticato, il cuore dell'applicazione è il loop di [`ui::main_ui::run`](client/src/ui/main_ui.rs): ad ogni iterazione ridisegna la schermata, poi si mette in attesa. Quest'ultima avviene tramite `tokio::select!`, che permette di reagire al primo evento pronto tra più sorgenti asincrone senza bisogno di un ciclo di polling attivo. In questo caso gli eventi in questione sono:
...
| File                                                                        | Ruolo                                                                                                                                                                                                                                                                                                                                          |
| --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`lib.rs`](client/src/lib.rs)                                               | Dichiara i sottomoduli `auth`, `listener`, `messaging`, `movement_sim`, `tools`, `ui`.                                                                                                                                                                                                                                                         |
| [`main.rs`](client/src/main.rs)                                             | Punto di ingresso: legge un file CSV per emulare successivamente il movimento, poi ripete in un `loop` l'intera sequenza connessione/canali/`writer`+`listener`+`movement_sim`/autenticazione/UI principale (3.2); il `loop` riparte automaticamente se l'utente elimina il proprio account, altrimenti il processo termina.                   |
| [`auth.rs`](client/src/auth.rs)                                             | Wrapper: inoltra `client_msg_tx`/`server_msg_rx` a `ui::auth_ui::run`.                                                                                                                                                                                                                                                                         |
| [`listener.rs`](client/src/listener.rs)                                     | Task di lettura: legge righe dal socket, deserializza `ServerMessage`, le inoltra su un canale `mpsc`. Dato che questo task è dedicato solo alla lettura, non viene mai bloccato da scrittura o UI.                                                                                                                                            |
...
| [`tools/movement_file_picker.rs`](client/src/tools/movement_file_picker.rs) | Seleziona a rotazione uno dei CSV in `movement_data/`, persistendo l'indice in `client/.movement_index`. L'indice di rotazione deve essere salvato in un file, così da valere anche tra esecuzioni diverse del processo e, quindi, per client diversi.                                                                                         |
...
| [`ui/auth_ui.rs`](client/src/ui/auth_ui.rs)                                 | Loop della schermata di login/registrazione: `tokio::select!` tra eventi tastiera e `AuthResult` dal server (inizializzazione in 3.3).                                                                                                                                                                                                         |
...
| [`ui/main_ui.rs`](client/src/ui/main_ui.rs)                                 | Loop della schermata principale: `tokio::select!` tra eventi tastiera, `server_msg_rx`, `movement_status_rx` e `client_error_rx` (inizializzazione in 3.3, loop in 3.4). Ritorna un `ExitReason` (`UserQuit` / `ConnectionLost` / `AccountDeleted`). In questo modo un solo `tokio::select!` multiplexa quattro fonti di eventi senza polling. |
